...
n_3 <-
  ...
  summarize(
    n = n(),
    n_src = n_distinct(title)
  ) %>%
  filter(n > 100) %>%
  mutate(
    n_words = str_count(ngram,"\\W+") + 1
  )
...
write_rds(n_grams,"../data/fathers_ngrams.rds")

```

```{r}
df <- 
  read_rds("../data/fathers.rds") %>%
  mutate(n = row_number())
n_grams <- 
  read_rds("../data/fathers_ngrams.rds") %>%
