...
pseudo_dionysius_1 <-
  readLines("https://ia800305.us.archive.org/30/items/worksofdionysius00pseu/worksofdionysius00pseu_djvu.txt",skipNul = T) %>% 
  as.data.frame() %>%
  rename_all(funs(gsub(.)), pattern = ".", replacement = "text") %>%
  slice(670:16739) %>%
  mutate(
...
pseudo_dionysius_2 <-
  readLines("https://ia800209.us.archive.org/7/items/worksofdionysius02pseu/worksofdionysius02pseu_djvu.txt",skipNul = T) %>% 
  as.data.frame() %>%
  rename_all(funs(gsub(.)), pattern = ".", replacement = "text") %>%
  slice(775:7005) %>%
  mutate(
