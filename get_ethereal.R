...
df <- 
  bind_rows(
    anf01,anf02,npnf07,npnf09,npnf13,
  ...
  mutate(chapter = if_else(str_detect(text,"^chapter "),text,NA_character_)) %>%
  fill(chapter, .direction = "down") %>%
  filter(!str_detect(text, "^chapter ")) %>%
  filter(!is.na(chapter))
  
rm(list = c("anf01","anf02","npnf07","npnf09","npnf13","pseudo_dionysius","pseudo_dionysius_2"))
